...
import OxfordDic from './Dictionary/oxfordDic';
...
*/
    ...
    const scope = urlParams.get('s') || 'ALL';
    ...
    useEffect(() => {
        ...
        Event('Dictionary', 'View Dictionary entry', 'Viewed dictionary entry for: '+word);
    }, [word])
    ...
    return (
        <div>
            ...
            <div className="card mt-4 pl-5 container p-4 d-flex justify-content-center align-items-start flex-column text-left">
                <h1 className="text-primary">{word} - definition</h1>

                <ReactGA.OutboundLink to={`/thesaurus?w=${word}`} eventLabel="Thesaurus"  target="_self" className="btn btn-white btn-sm topbtn">
                 Synonyms
                </ReactGA.OutboundLink>
                ...
                {!notFound && <div>
                    ...
                    {(oxfordResult && oxfordResult.length > 0) &&
                        <OxfordDic oxfordResult={oxfordResult} />
                    }
                </div>}
            </div>
        </div>
